...
from utility.static import now, timedelta_sec, get_binance_short_profit, get_binance_long_profit, str_ymd, str_hms, \
...
class BinanceTrader:
    def __init__(self, qlist, dict_set):
        ...
        if not self.dict_set['모의투자']:
            ...
            self.ws_thread.signal1.connect(self._update_user_data)
            self.ws_thread.start()

        self.qtimer1 = QTimer()
        self.qtimer1.setInterval(500)
        self.qtimer1.timeout.connect(self.__scheduler1)
        self.qtimer1.start()

        self.qtimer2 = QTimer()
        self.qtimer2.setInterval(1 * 1000)
        self.qtimer2.timeout.connect(self.__scheduler2)
        self.qtimer2.start()
    ...
    def _set_position(self):
        ...
        self.dict_info = {
            x['symbol']: {
                '가격소숫점자리수': get_decimal_place(x['filters'][0]['tickSize']),
                '수량소숫점자리수': get_decimal_place(x['filters'][2]['minQty']),
                '시드부족시간': dummy_time,
                '최종거래시간': dummy_time,
                '손절거래시간': dummy_time
            } for x in datas
        ...
        self.stgQ.put(('바낸선물단위정보', self.dict_info))
    ...
    def __scheduler1(self):
        self.stgQ.put(('잔고목록', self.dict_jg.copy()))

    def __scheduler2(self):
    ...
    def _check_order(self, data):
        ...
        if 잔고청산:
            ...
            if self.dict_set['매수금지거래횟수'] and self.dict_set['매수금지거래횟수값'] <= 거래횟수:
                ...
                if 현재시간 > self.dict_info[종목코드]['시드부족시간']:
                    self._create_order('시드부족', 종목코드, 주문가격, 주문수량, str_hmsf(), 시그널시간, 잔고청산, 0, None)
        ...
        if 주문취소:
            if 'CANCEL' not in 주문구분:
                self.stgQ.put((f'{주문구분}_CANCEL', 종목코드))
        else:
            if 주문수량 > 0:
                if 잔고청산: self.stgQ.put((f'{주문구분}_MANUAL', 종목코드))
    ...
    def _order_timeLog(self, signal_time):
    ...
    def _send_order(self, data):
        ...
        if 'CANCEL' not in 주문구분:
            try:
                ...
                self._update_chegeollist(dt, 종목코드, f'{주문구분}_REG', 주문수량, 0, 주문수량, 0, dt[:14], 주문가격, orderId)
    ...
    def _update_jango(self, data):
        ...
        try:
            ...
            if 현재가 != self.dict_jg[종목코드]['현재가']:
                ...
                if 포지션 == 'LONG':
                    평가금액, 평가손익, 수익률 = get_binance_long_profit(매입금액, 보유수량 * 현재가, '시장가' in self.dict_set['매수주문구분'], '시장가' in self.dict_set['매도주문구분'])
                else:
                    평가금액, 평가손익, 수익률 = get_binance_short_profit(매입금액, 보유수량 * 현재가, '시장가' in self.dict_set['매수주문구분'], '시장가' in self.dict_set['매도주문구분'])
        ...
        for code in self.dict_info:
            ...
            if lhp:
                try:
                    ...
                    leverage = self._get_leverage(lhp)
    ...
    def _cancel_order(self, 종목코드, 주문구분):
        ...
        if last_value:
            ...
            if 미체결수량 > 0:
                ...
                self._create_order(f'{주문구분}_CANCEL', 종목코드, 주문가격, 미체결수량, 주문번호, now(), False, 0, None)
    ...
    def _modify_order(self, 종목코드, 주문구분):
        ...
        if last_value:
            ...
            if 미체결수량 > 0:
                ...
                주문번호, 주문가격 = last_value['주문번호'], last_value['주문가격']
                self._create_order(f'{주문구분}_CANCEL', 종목코드, 주문가격, 미체결수량, 주문번호, 현재시간, False, 0, None)
    ...
    def _jango_cheongsan(self, gubun):
        ...
        if self.dict_jg:
            ...
            for 종목코드 in self.dict_jg.copy():
                ...
                if self.dict_set['모의투자']:
                    self._update_chejan_data(주문구분, 종목코드, 보유수량, 보유수량, 0, 현재가, 현재가, '')
                else:
        ...
        if data['e'] == 'ACCOUNT_UPDATE':
            ...
            try:
                ...
                if cc > 0 or 'CANCEL' in p:
                    self._update_chejan_data(p, code, oc, cc, mc, cp, op, on)
        ...
        if 주문구분 in ('BUY_LONG', 'SELL_SHORT', 'SELL_LONG', 'BUY_SHORT'):
            if 주문구분 in ('BUY_LONG', 'SELL_SHORT'):
                ...
                if 종목코드 in self.dict_jg:
                    ...
                    if 주문구분 == 'BUY_LONG':
                        평가금액, 수익금, 수익률 = get_binance_long_profit(매입금액, 평가금액, '시장가' in self.dict_set['매수주문구분'], '시장가' in self.dict_set['매도주문구분'])
                    else:
                        평가금액, 수익금, 수익률 = get_binance_short_profit(매입금액, 평가금액, '시장가' in self.dict_set['매수주문구분'], '시장가' in self.dict_set['매도주문구분'])
                    self.dict_jg[종목코드].update({
                        '매수가': 매수가,
                        '현재가': 체결가격,
                        '수익률': 수익률,
                    ...
                    if 주문구분 == 'BUY_LONG':
                        포지션 = 'LONG'
                        평가금액, 수익금, 수익률 = get_binance_long_profit(매입금액, 매입금액, '시장가' in self.dict_set['매수주문구분'], '시장가' in self.dict_set['매도주문구분'])
                    else:
                        포지션 = 'SHORT'
                        평가금액, 수익금, 수익률 = get_binance_short_profit(매입금액, 매입금액, '시장가' in self.dict_set['매수주문구분'], '시장가' in self.dict_set['매도주문구분'])
                    self.dict_jg[종목코드] = {
                        '종목명': 종목코드,
                        '포지션': 포지션,
                        '매수가': 체결가격,
                ...
                if 주문구분 == 'SELL_LONG':
                    ...
                    평가금액, 수익금, 수익률 = get_binance_short_profit(매입금액, 평가금액, '시장가' in self.dict_set['매수주문구분'], '시장가' in self.dict_set['매도주문구분'])
                if -100 < 수익률 < 100: self._update_tradelist(index, 종목코드, 포지션, 매입금액, 평가금액, 체결수량, 수익률, 수익금, index[:14])
            ...
            if 미체결수량 == 0: self.stgQ.put((f'{주문구분}_COMPLETE', 종목코드))
    ...
    def _update_tradelist(self, index, 종목코드, 포지션, 매입금액, 평가금액, 체결수량, 수익률, 수익금, 주문시간):

...
from utility.static import now, now_utc, dt_ymdhms, get_hogaunit_coin, get_profit_coin, get_indicator
        ...
        if 전략연산:
            ...
            if 데이터길이 >= self.rolling_window:
                ...
                if jg_data:
                    ...
                    _, 수익금, 수익률 = get_profit_coin(매입금액, 보유수량 * 현재가)
                ...
                G = NIB and NIS and 매수가 != 0 and \
                    self.dict_set['매도손절수익률청산'] and 수익률 < -self.dict_set['매도손절수익률']
                H = NIB and NIS and 매수가 != 0 and \

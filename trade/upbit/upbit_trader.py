...
class UpbitTrader:
    ...
    def LoadDatabase(self):
        ...
        df_td = get_pd().read_sql(f"SELECT * FROM c_tradelist WHERE 체결시간 LIKE '{self.str_today}%'", con).set_index('index')
        if len(df_cj) > 0:
        ...
        if self.dict_set['코인모의투자']:
            df_jg = get_pd().read_sql(f'SELECT * FROM c_jangolist', con).set_index('index')
            if len(df_jg) > 0:
    ...
    def GetBalances(self):
        if self.dict_set['코인모의투자']:
            con = sqlite3.connect(DB_TRADELIST)
            df = get_pd().read_sql('SELECT * FROM c_tradelist', con)
            con.close()
            tcg = df['수익금'].sum()
    ...
    def Scheduler1(self):
        ...
        if not self.dict_set['코인모의투자']: self.CheckChegeol()
    ...
    def UpdateTradelist(self, index, 종목코드, 매입금액, 평가금액, 체결수량, 수익률, 수익금, 주문시간):
        ...
        df_td = get_pd().DataFrame.from_dict(self.dict_td, orient='index')
        self.windowQ.put((ui_num['C거래목록'], df_td[::-1]))
        df = get_pd().DataFrame([[종목코드, 매입금액, 평가금액, 체결수량, 수익률, 수익금, 주문시간]], columns=columns_td, index=[index])

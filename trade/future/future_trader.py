...
from utility.static import now, timedelta_sec, get_future_long_profit, get_future_short_profit, error_decorator
...
class FutureTrader(BaseTrader):
    ...
    def _get_balances(self):
        if self.dict_set['모의투자']:
            ...
            yesugm = self.ls.get_balance_future()
    ...
    def _send_order(self, data):
        ...
        if 'CANCEL' not in 주문구분:
            ...
            od_no, msg = self.ls.order_future(종목코드, 주문구분, 주문가격, 주문수량, 주문유형)
            if od_no != '0':
            ...
            od_no, msg = self.ls.order_modify_future(종목코드, 원주문번호, 주문가격, 주문수량, 주문유형)
            if od_no == '0':
                self.windowQ.put((
                    ui_num['기본로그'], f'주문 관리 시스템 알림 - [{주문구분} 실패] {종목명} | {주문가격} | {주문수량}'
                ))
            ...
            od_no, msg = self.ls.order_cancel_future(종목코드, 원주문번호, 주문수량)
            if od_no == '0':
                self.windowQ.put((
                    ui_num['기본로그'], f'주문 관리 시스템 알림 - [{주문구분} 실패] {종목명} | {주문가격} | {주문수량}'
                ))

...
class FutureTrader:
    ...
    def _update_string(self, data):
        if data == '체결목록':
            ...
            self.teleQ.put(df_cj if len(df_cj) > 0 else '현재는 해선 체결목록이 없습니다.')
        elif data == '거래목록':
            df_td = pd.DataFrame.from_dict(self.dict_td, orient='index')
            self.teleQ.put(df_td if len(df_td) > 0 else '현재는 해선 거래목록이 없습니다.')
        elif data == '잔고평가':
            df_jg = pd.DataFrame.from_dict(self.dict_jg, orient='index')
            self.teleQ.put(df_jg if len(df_jg) > 0 else '현재는 해선 잔고목록이 없습니다.')
        elif data == '잔고청산':
    ...
    def _jango_cheongsan(self, gubun):
        ...
        if self.dict_jg:
            ...
            if self.dict_set['알림소리']:
                self.soundQ.put('해선 잔고청산 주문을 전송하였습니다.')
    ...
    def _update_chejan_data(self, data):
        ...
        if 주문상태 == '접수' and 주문구분 == '신규' and 매도수구분 in ('매수', '매도'):
            ...
            self.windowQ.put((ui_num['기본로그'], f'주문 관리 시스템 알림 - [{gubun}_REG] {종목명} | {주문가격} | {주문수량}'))
            ...
            self.windowQ.put((ui_num['기본로그'], f'주문 관리 시스템 알림 - [{주문상태}][{gubun}] {종목명} | {주문가격} | {주문수량}'))
    ...
    def _update_tradelist(self, index, 종목명, 포지션, 매입금액, 평가금액, 체결수량, 수익률, 수익금, 주문시간):
        ...
        self.queryQ.put(('거래디비', df, 'f_tradelist', 'append'))
    ...
    def _update_totaltradelist(self, first=False):
        ...
        self.queryQ.put(('거래디비', df_tt, 'f_totaltradelist', 'append'))
    ...
    def _update_chegeollist(self, index, 종목코드, 종목명, 주문구분, 주문수량, 체결수량, 미체결수량, 체결가격, 체결시간, 주문가격, 주문번호):
        ...
        self.queryQ.put(('거래디비', df, 'f_chegeollist', 'append'))

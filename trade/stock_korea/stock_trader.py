...
from utility.static import now, timedelta_sec, str_hms, get_stock_profit, get_stock_hogaunit, str_ymd, str_ymdhms, \
    str_ymdhmsf, dt_hms, qtest_qwait, get_profile_text, set_builtin_print, error_decorator
...
class StockTrader:
    ...
    def _load_database(self):
        ...
        con.close()
        self.windowQ.put((ui_num['기본로그'], '시스템 명령 실행 알림 - 데이터베이스 정보 불러오기 완료'))
        ...
        if 잔고청산:
            ...
            if self.dict_set['매수금지거래횟수'] and self.dict_set['매수금지거래횟수값'] <= 거래횟수:
                ...
                if 현재시간 > self.dict_info[종목코드]['시드부족시간']:
                    self._create_order('시드부족', 종목코드, 종목명, 주문가격, 주문수량, '', 시그널시간, 잔고청산, None)
    ...
    def _create_order(self, 주문구분, 종목코드, 종목명, 주문가격, 주문수량, 원주문번호, 시그널시간, 잔고청산, 수동주문유형):
        ...
        if 잔고청산:
            if not (self.dict_set['모의투자'] or 주문구분 == '시드부족'):
                주문가격 = 0
        elif 주문구분 == '매수':
            if self.dict_set['매수주문구분'] in ('지정가', '지정가IOC', '지정가FOK'):
                주문가격 += get_stock_hogaunit(주문가격) * self.dict_set['매수지정가호가번호']
            if self.dict_set['매수주문구분'] not in ('지정가', '지정가IOC', '지정가FOK'):
                if not (self.dict_set['모의투자'] or 주문구분 == '시드부족'):
                    주문가격 = 0
        elif 주문구분 == '매도':
            if self.dict_set['매도주문구분'] in ('지정가', '지정가IOC', '지정가FOK'):
                주문가격 += get_stock_hogaunit(주문가격) * self.dict_set['매도지정가호가번호']
            if self.dict_set['매도주문구분'] not in ('지정가', '지정가IOC', '지정가FOK'):
                if not (self.dict_set['모의투자'] or 주문구분 == '시드부족'):
                    주문가격 = 0
    ...
    def _update_tuple(self, data):
        ...
        if gubun == '체잔통보':
            ...
            self._put_order_complete('매수취소', data)
        elif gubun == '잔고조회':
    ...
    def _update_string(self, data):
        if data == '체결목록':
            ...
            self.windowQ.put(('tele', df_cj)) if len(df_cj) > 0 else self.windowQ.put(('tele', '현재는 주식 체결목록이 없습니다.'))
        elif data == '거래목록':
            df_td = pd.DataFrame.from_dict(self.dict_td, orient='index')
            self.windowQ.put(('tele', df_td)) if len(df_td) > 0 else self.windowQ.put(('tele', '현재는 주식 거래목록이 없습니다.'))
        elif data == '잔고평가':
            df_jg = pd.DataFrame.from_dict(self.dict_jg, orient='index')
            self.windowQ.put(('tele', df_jg)) if len(df_jg) > 0 else self.windowQ.put(('tele', '현재는 주식 잔고목록이 없습니다.'))
        elif data == '잔고청산':
    ...
    def _update_Jango(self, data):
        ...
        try:
            if 현재가 != self.dict_jg[종목코드]['현재가']:
                ...
                평가금액, 평가손익, 수익률 = get_stock_profit(매입금액, 보유수량 * 현재가)
                ...
                if code_ is None or code == code_:
                    ...
                    if gubun == '매수':
                        ...
                        if self.dict_set['매도취소시간'] and now() > order_info[0]:
                            cancel_list.append((code, gubun))
                    if gubun == '매수':
                        if order_info[1] < self.dict_set['매수정정횟수'] and code in self.dict_curc and \
                                self.dict_curc[code] >= order_info[2] + order_info[3] * self.dict_set['매수정정호가차이']:
                            modify_list.append((code, gubun))
                    else:
                        if order_info[1] < self.dict_set['매도정정횟수'] and code in self.dict_curc and \
                                self.dict_curc[code] <= order_info[2] - order_info[3] * self.dict_set['매도정정호가차이']:
                            modify_list.append((code, gubun))
    ...
    def _cancel_order(self, 종목코드, 주문구분):
        ...
        if last_value:
            ...
            if 미체결수량 > 0:
                ...
                self._create_order(f'{주문구분}취소', 종목코드, 종목명, 0, 미체결수량, 주문번호, 현재시간, False, None)
    ...
    def _modify_order(self, 종목코드, 주문구분):
        ...
        if last_value:
            ...
            if 미체결수량 > 0:
                if 주문구분 == '매수':
                    주문가격 = self.dict_curc[종목코드] - self.dict_order[주문구분][종목코드][3] * self.dict_set[f'{주문구분}정정호가']
                else:
                    주문가격 = self.dict_curc[종목코드] + self.dict_order[주문구분][종목코드][3] * self.dict_set[f'{주문구분}정정호가']

                현재시간 = now()
                주문번호 = last_value['주문번호']
                self._create_order(f'{주문구분}정정', 종목코드, 종목명, 주문가격, 미체결수량, 주문번호, 현재시간, False, None)
    ...
    def _jango_cheongsan(self, gubun):
        ...
        if self.dict_jg:
            if gubun == '수동':
                self.windowQ.put(('tele', ' 잔고청산 주문을 전송합니다.'))
            ...
            self.windowQ.put((ui_num['기본로그'], f'시스템 명령 실행 알림 - 주식 잔고청산 주문 완료'))
        elif gubun == '수동':
            self.windowQ.put(('tele', '현재는 주식 보유종목이 없습니다.'))
    ...
    def _update_chejan_data(self, data):
        ...
        if 주문상태 == '접수' and 주문구분 in ('매수', '매도'):
            cancel_time = timedelta_sec(self.dict_set['매수취소시간초' if 주문구분 == '매수' else '매도취소시간초'])
            if 주문구분 == '매수':
                self.dict_intg['추정예수금'] -= 주문수량 * (주문가격 if '지정가' in self.dict_set['매수주문구분'] else 최우선매도호가)
                self.dict_order[주문구분][종목코드] = [cancel_time, 0, 주문가격, get_stock_hogaunit(주문가격)]
            else:
                self.dict_order[주문구분][종목코드] = [cancel_time, 0, 주문가격, get_stock_hogaunit(주문가격)]
            self._update_chegeollist(index, 종목코드, 종목명, f'{주문구분} {주문상태}', 주문수량, 체결수량, 미체결수량, 체결가격, 주문시간, 주문가격, 주문번호)
            self.windowQ.put((ui_num['기본로그'], f'주문 관리 시스템 알림 - [{주문구분}{주문상태}] {종목명} | {주문가격} | {주문수량}'))
            ...
            if 주문구분 == '매수':
                ...
                if 종목코드 in self.dict_jg:
                    ...
                    평가금액, 수익금, 수익률 = get_stock_profit(매입금액, 보유수량 * 체결가격)
                    self.dict_jg[종목코드].update({
                        '매수가': 매수가,
                        '현재가': 체결가격,
                        '수익률': 수익률,
                ...
                매입금액 = 매수가 * 체결수량
                평가금액, 수익금, 수익률 = get_stock_profit(매입금액, 체결수량 * 체결가격)
            ...
            if 미체결수량 == 0: self._put_order_complete(주문구분 + '완료', 종목코드)
            self._update_chegeollist(index, 종목코드, 종목명, 주문구분, 주문수량, 체결수량, 미체결수량, 체결가격, 주문시간, 주문가격, 주문번호)
            ...
            df_jg = pd.DataFrame.from_dict(self.dict_jg, orient='index')
            self.windowQ.put(('거래디비', df_jg, 's_jangolist', 'replace'))
            if self.dict_set['알림소리']: self.soundQ.put(f'{종목명} {체결수량}주를 {주문구분}하였습니다')
            self.windowQ.put((ui_num['기본로그'], f'주문 관리 시스템 알림 - [{주문구분}{주문상태}] {종목명} | {체결가격} | {체결수량}'))
            ...
            if 주문구분 in ('매수정정', '매도정정'):
                ...
                self.dict_order[주문구분_][종목코드] = [취소시간, 정정횟수, 주문가격, get_stock_hogaunit(주문가격)]
            else:
                if 주문구분 == '매수취소':
    ...
    def _update_tradelist(self, index, 종목명, 매입금액, 평가금액, 체결수량, 수익률, 수익금, 주문시간):
        ...
        self.windowQ.put(('거래디비', df, 's_tradelist', 'append'))
    ...
    def _update_totaltradelist(self, first=False):
        ...
        self.windowQ.put(('거래디비', delete_query))
        self.windowQ.put(('거래디비', df_tt, 's_totaltradelist', 'append'))
        self.windowQ.put((ui_num['S실현손익'], df_tt))
        ...
        if self.dict_set['스톰라이브']:
            ...
            self.windowQ.put(('live', ('', data_list)))
    ...
    def _update_chegeollist(self, index, 종목코드, 종목명, 주문구분, 주문수량, 체결수량, 미체결수량, 체결가격, 체결시간, 주문가격, 주문번호):
        ...
        self.windowQ.put(('거래디비', df, 's_chegeollist', 'append'))
    ...
    def _update_totaljango(self):
        ...
        if self.dict_set['투자금고정']:
            ...
            if '시장가' in self.dict_set['매수주문구분']:
                종목당투자금 = int((self.dict_intg['추정예탁자산'] - self.dict_intg['추정예탁자산'] / self.dict_set['최대매수종목수'] * 0.3) / self.dict_set['최대매수종목수'])
            else:
        ...
        self.windowQ.put((ui_num['S잔고목록'], df_jg))
        self.windowQ.put((ui_num['S잔고평가'], df_tj))

...
from utility.static import now, timedelta_sec, get_stock_profit, get_stock_hogaunit, error_decorator
...
class StockTrader(BaseTrader):
    ...
    def _cancel_order(self, 종목코드, 주문구분):
        ...
        if last_value:
            ...
            if 미체결수량 > 0:
                ...
                self._create_order(f'{주문구분}취소', 종목코드, 종목명, 0, 미체결수량, 주문번호, 현재시간, False, 0, None)
    ...
    def _jango_cheongsan(self, gubun):
        ...
        if self.dict_jg:
            ...
            self.teleQ.put('현재는 주식 보유종목이 없습니다.')

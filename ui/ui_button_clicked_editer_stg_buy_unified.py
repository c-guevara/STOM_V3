...
# =============================================================================
# UI 타입별 설정 매핑 (STG BUY 에디터용)
# =============================================================================
UI_STG_BUY_EDITER_CONFIG = {
    'coin': {
        'widgets': {
            'combo': 'cvjb_comboBoxx_01',
            'line': 'cvjb_lineEditt_01',
            'text': 'cs_textEditttt_01',
            'push_04': 'cvjb_pushButon_04',
            'push_12': 'cvjb_pushButon_12',
        },
        ...
        'signal_fn': lambda ui: buy_signal if '업비트' in ui.dict_set['거래소'] else future_buy_signal,
    },
    'stock': {
        'widgets': {
            'combo': 'svjb_comboBoxx_01',
            'line': 'svjb_lineEditt_01',
            'text': 'ss_textEditttt_01',
            'push_04': 'svjb_pushButon_04',
            'push_12': 'svjb_pushButon_12',
        },
        'gubun_fn': lambda ui: 'stock' if '키움증권' in ui.dict_set['증권사'] else 'future',
        'is_stock_fn': lambda ui: '키움증권' in ui.dict_set['증권사'],
        'tf_key': '타임프레임',
        'table_fn': lambda gubun: f'{gubun}buy',
        'queue': 'wdzservQ',
        'need_process_check': False,
        'signal_fn': lambda ui: buy_signal if '키움증권' in ui.dict_set['증권사'] else future_buy_signal,
    }
}
...
def _get_widget(ui, ui_type, widget_key):
    ...
    widget_name = UI_STG_BUY_EDITER_CONFIG[ui_type]['widgets'][widget_key]
    return getattr(ui, widget_name)


def _get_gubun(ui, ui_type):
    """UI 타입에 따른 gubun 반환"""
    return UI_STG_BUY_EDITER_CONFIG[ui_type]['gubun_fn'](ui)


def _get_table_name(ui_type, gubun=None):
    """UI 타입에 따른 테이블명 반환"""
    config = UI_STG_BUY_EDITER_CONFIG[ui_type]
    if ui_type == 'coin':
        return config['table']
    else:
...
def _get_queue(ui, ui_type):
    """UI 타입에 따른 큐 객체 반환"""
    queue_name = UI_STG_BUY_EDITER_CONFIG[ui_type]['queue']
...
# =============================================================================
@error_decorator
def buy_factor(ui, ui_type):
    """매수 팩터 템플릿 삽입 (코인/주식 공통)"""
    config = UI_STG_BUY_EDITER_CONFIG[ui_type]
    text_widget = _get_widget(ui, ui_type, 'text')
    push_04 = _get_widget(ui, ui_type, 'push_04')
    ...
    text_widget.clear()
    text_widget.append(buy_text_tick if ui.dict_set[tf_key] else buy_text_min)
    push_04.setStyleSheet(style_bc_st)


@error_decorator
def buy_signal_insert(ui, ui_type):
    """매수 시그널 삽입 (코인/주식 공통)"""
    config = UI_STG_BUY_EDITER_CONFIG[ui_type]
    text_widget = _get_widget(ui, ui_type, 'text')
...
# =============================================================================
@error_decorator
def buy_stg_start(ui, ui_type):
    """매수 전략 시작 (코인/주식 공통)"""
    text_widget = _get_widget(ui, ui_type, 'text')
    push_04 = _get_widget(ui, ui_type, 'push_04')
    push_12 = _get_widget(ui, ui_type, 'push_12')
        ...
        if buttonReply == QMessageBox.Yes:
            if ui_type == 'coin':
                if strategy_process_alive(ui):
                    queue.put(('매수전략', strategy))
            else:
                queue.put(('strategy', ('매수전략', strategy)))
            push_04.setStyleSheet(style_bc_dk)
            push_12.setStyleSheet(style_bc_st)


@error_decorator
def buy_stg_stop(ui, ui_type):
    """매수 전략 중지 (코인/주식 공통)"""
    push_04 = _get_widget(ui, ui_type, 'push_04')
    push_12 = _get_widget(ui, ui_type, 'push_12')
    queue = _get_queue(ui, ui_type)
    ...
    if ui_type == 'coin':
        ...
        queue.put(('strategy', '매수전략중지'))

    push_12.setStyleSheet(style_bc_dk)
    push_04.setStyleSheet(style_bc_st)

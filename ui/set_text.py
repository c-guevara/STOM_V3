...
# 국내주식에만 있는 'N(이전틱수)' 호출 가능 변수들
거래대금증감, 전일비, 회전율, 전일동시간비, 시가총액, 라운드피겨위5호가이내(boolean)
# 국내주식에만 있는 '변수명(구간틱수, 이전틱수)' 호출 가능 변수
전일비각도
...
평균값계산틱수 : 변수들의 평균, 최저, 최고 등의 값을 계산하는 최소 틱수
...
초당매수금액 : 초당기록된 매수수량 * 현재가
초당매도금액 : 초당기록된 매수수량 * 현재가
...
buy_text_min = '''"""
# 기본 변수들 - 괄호없이 변수명만으로 사용한다.
# 변수명 뒤에 'N(이전분봉수)'을 붙여 이전값을 조회할 수 있다 예: 현재가N(1) - 1틱전 현재가
...
평균값계산틱수 : 변수들의 평균, 최저, 최고 등의 값을 계산하는 최소 분봉수
매수 : True 상태의 변수이다. 전략 상단에 매수 = False로 시작하면 참인 조건들을 and로 묶어서 전략을 작성할 수 있다.
...
고저평균대비등락율 : 당일 고가와 저가의 평균 대비 현재가의 등락율
저가대비고가등락율 : 당일 저가 대비 당일 고가의 등락율
분당매수금액 : 분당기록된 매수수량 * 현재가
분당매도금액 : 분당기록된 매수수량 * 현재가
분당매수금액 : 분당기록된 매수수량 * 현재가
분당매도금액 : 분당기록된 매수수량 * 현재가
...
회전율 : 상장주식수 대비 당일거래량의 비율 (당일거래량 / 상장주식수 * 100)
전일동시간비 : 1분단위로 누적 기록된 전일 동시간대의 거래량 대비 당일거래량의 비율 (당일거래량 / 전일동시간거래량 * 100)
전일비각도 : 입력한 구간분봉수 동안의 전일비 차이를 높이로 하고 분봉수를 밑변으로 계산한 각도 (0~90)
거래대금증감 : 전일거래대금 보다 증가한 당일거래대금 (당일거래대금 - 전일거래대금)
라운드피겨위5호가이내 : 라운드피겨 가격(예: 1000)포함 상위 5호가(예: 1025)이내에 현재가가 위치하면 True 아니면 False
매도수5호가잔량합 : 매수, 매도잔량 1호가부터 5호가까지의 총합
VI해제시간, VI가격, VI호가단위 : VI해제시간, 상승VI가격, 상승VI가격의 호가단위
...
self.indicator['ADXR_timeperiod'] = self.vars[7]
...
example_vars2 = '''# 작성방법은 최적화 범위 코드와 유사합니다.
# 벨류값은 범위리스트와 고정값으로 구분되며
# 범위리스트는 단순나열형태, 고정값은 하나의 수를
# 입력해야합니다. 이 고정값은 2단계, 3단계에서
# 이전 탐색에서 검출되지 않아도 돌연변이로 추가됩니다.
...
example_vars3 = '''# 작성방법은 최적화 범위 코드와 유사합니다.
# 벨류값은 범위리스트와 고정값으로 구분되며
# 범위리스트는 단순나열형태, 고정값은 하나의 수를
# 입력해야합니다. 이 고정값은 2단계, 3단계에서
# 이전 탐색에서 검출되지 않아도 돌연변이로 추가됩니다.
...
self.vars[5] = [[0, -1, -2, -3, -4, -5], 0]
self.vars[6] = [[120, 110, 100, 90, 80], 100]
self.vars[7] = [[14, 15, 16, 17, 18, 19], 1]
self.vars[8] = [[-1, -2, -3, -4, -5], -1]
self.vars[9] = [[1, 2, 3, 4, 5], 2]
self.vars[10] = [[-1, -2, -3, -4, -5], -1]
self.vars[11] = [[1, 2, 3, 4, 5], 2]'''
...
example_opti_vars2 = '''# 키값 0번을 제외한 모든 변수는 최적화편집기에서 사용한 변수들입니다.
...
example_opti_vars3 = '''# 키값 0번을 제외한 모든 변수는 최적화편집기에서 사용한 변수들입니다.
# 키값 0번의 벨류값은 평균값 계산틱수를 입력하십시오.

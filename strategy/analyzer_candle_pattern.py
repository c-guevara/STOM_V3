...
class AnalyzerCandlePattern:
    ...
    def __init__(self, market_gubun: int, market_info: dict, backtest: bool = False, min_samples: int = 20):
        ...
        """
        self.analysis_period, self.rate_threshold = _load_pattern_setting(market_gubun)
        self.setting_hash     = calculate_setting_hash(self.analysis_period, self.rate_threshold)
        self.pattern_database = CandlePatternDatabase(market_info['전략구분'], self.setting_hash)
        ...
        with sqlite3.connect(PATTERN_DB) as conn:
            ...
            for pattern_name, scores in pattern_scores.items():
                cursor.execute(f'''
                    ...
                    (code, pattern_name, setting_hash, avg_score, max_score, min_score, std_score, sample_count,
...
def _load_pattern_setting(market: int) -> tuple:
    """
    마켓번호로 설정값 불러오기
    market: 마켓번호 (1~9)
...
def _save_pattern_setting(market: int, analysis_period: int, rate_threshold: int):
    """
    마켓번호로 설정값 저장
    market: 마켓번호 (1~9)
...
def pattern_setting_load(ui):
    ...
    analysis_period, rate_threshold = _load_pattern_setting(ui.market_gubun)
...
def pattern_setting_save(ui):
    ...
    _save_pattern_setting(ui.market_gubun, analysis_period, rate_threshold)

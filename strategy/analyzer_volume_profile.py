...
class AnalyzerVolumeProfile:
    ...
    def __init__(self, market_gubun: int, market_info: dict, backtest: bool = False, top_nodes: int = 20):
        ...
        """
        self.analysis_period, self.rate_threshold, self.price_range_pct = _load_volume_setting(market_gubun)
        self.setting_hash = calculate_setting_hash(self.analysis_period, self.rate_threshold, self.price_range_pct)
        self.volume_database = VolumeProfileDatabase(market_info['전략구분'], self.setting_hash)
    ...
    def train_all_codes(self, windowQ):
        ...
        with Pool(processes=actual_processes, initializer=init_worker, initargs=(windowQ,)) as pool:
            args = [
                (
                    i, chunk, self.backtest_db, VOLUME_PROFILE_DB, self.idx_close, self.idx_volume,
        ...
        for k, code in enumerate(code_chunk):
            try:
                ...
                with sqlite3.connect(pattern_db) as conn:
                    cursor = conn.cursor()
                    cursor.execute(f'SELECT DISTINCT last_update FROM volume_score WHERE code = ?', (code,))
        ...
        with sqlite3.connect(VOLUME_PROFILE_DB) as conn:
            ...
            for price_level, scores in volume_scores.items():
                cursor.execute(f'''
                    ...
                    (code, price_level, setting_hash, avg_score, upward_strength, downward_strength, sample_count,
...
def _load_volume_setting(market: int) -> tuple:
    """
    마켓번호로 설정값 불러오기
    market: 마켓번호 (1~9)
...
def _save_volume_setting(market: int, analysis_period: int, rate_threshold: float, price_range_pct: float):
    """
    마켓번호로 설정값 저장
    market: 마켓번호 (1~9)
...
def volume_setting_load(ui):
    ...
    analysis_period, rate_threshold, price_range_pct = _load_volume_setting(ui.market_gubun)
...
def volume_setting_save(ui):
    ...
    _save_volume_setting(ui.market_gubun, analysis_period, rate_threshold, price_range_pct)
